...
export class ChatMessageEntity {
  ...
  static fromSendForm(form: ChatMessageSendForm, fromAccount: AccountEntity): ChatMessageEntity {
    const date = DateTime.now()
    const entity = new ChatMessageEntity(form.recipientAccountId, fromAccount.uid, form.text, date, undefined, true)
    return entity
  }

  static fromModel(model: ChatMessageModel): ChatMessageEntity {
    const date = DateTime.fromFormat(model.sentDate, "YYYY-MM-DD[T]HH:mm:ss.SSSZZ")
    const entity = new ChatMessageEntity(model.toAccountId, model.fromAccountId, model.text, date, model.id, model.read)
    return entity
  }
  ...
  socketString = () =>
    `${this.fromAccountId}::${this.toAccountId}::${this.sentDate.toFormat("YYYY-MM-DD[T]HH:mm:ss.SSSZZ")}::${this.text}`
}
